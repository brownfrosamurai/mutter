...
| "Multi-language — Yoruba and most common languages" | English, Yoruba, Spanish, Italian, French, Arabic at minimum | Yoruba is a low-resource language for Whisper; expect materially worse accuracy than the other five unless benchmarked and mitigated — see Section 6. **Status, 2026-08-30: Yoruba and the other four non-English languages are parked by the user's own direction; v1 build/validation focus is English only for now — see Section 6's status note.** |
...
**Status, 2026-08-30:** the user has parked the Yoruba requirement for now and directed v1 to focus solely on English — a real scope decision, not a silent drop. Practical effect on this section: the Phase 0 fork is resolved for the languages actually in scope — Whisper Small already measured 100% accuracy on English (`src-tauri/tests/language_benchmark.rs`), so per this plan's own fork logic ("if Whisper wins, `AppleSpeechEngine` implementation work is skipped entirely," Section 12/Roadmap), `AppleSpeechEngine` is not being built. Nothing in the shipped architecture is English-only, though — `WhisperEngine` is still the multilingual GGML build with auto-detection (Section 10), so Spanish/Italian/French/Arabic/Yoruba keep working exactly as before if someone dictates in them; only the *dedicated benchmarking and accuracy-hardening work* for the other five languages is on hold, revisited if/when Yoruba (or the others) come back into scope.
...
**Further status, 2026-08-30 (same day):** the user has since parked Yoruba and narrowed v1 focus to English only — see Section 6's status note. Item 1 of this assignment is therefore considered closed for the languages currently in scope: English's result is real and decisive (100% on Whisper Small), so there's no open engine-choice question left to resolve by ear. The Yoruba/six-language version of this assignment is on hold, not abandoned — pick it back up if that scope returns.
