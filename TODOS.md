...
**2026-09-02: every open item below now has a matching GitHub issue** (filed
so outside contributors have something concrete to pick up — this file
stays the detailed historical record; the issue is the place to actually
comment/claim/discuss). If you start work on one, comment on its issue
first and update/remove the entry here when done, so the two don't drift.

## Onboarding Ready screen: missing ARIA live region for permission status — [issue #7](https://github.com/brownfrosamurai/mutter/issues/7)
...
## Onboarding/Recovery chromeless migration compounds with the still-unverified mic prompt path — no separate issue, tracked under [issue #15](https://github.com/brownfrosamurai/mutter/issues/15)
...
## Ready.tsx's resolved phase may overflow the fixed 480×480 onboarding window — [issue #8](https://github.com/brownfrosamurai/mutter/issues/8)
...
## Onboarding's permission auto-fire and Settings' Grant button can race for the same permission kind — [issue #9](https://github.com/brownfrosamurai/mutter/issues/9)
...
## Tray Quit bypasses the onboarding permission-request busy-guard — [issue #10](https://github.com/brownfrosamurai/mutter/issues/10)
...
## Unverified: do Accessibility/Screen Recording's native calls block the whole app, not just the requesting window? — [issue #11](https://github.com/brownfrosamurai/mutter/issues/11)
...
## PARKED — Yoruba accuracy benchmark (Section 6 / Section 17) — [issue #12](https://github.com/brownfrosamurai/mutter/issues/12) (see also [issue #5](https://github.com/brownfrosamurai/mutter/issues/5) for the automatable Spanish/Hindi/Italian/French/Arabic version of this benchmark)
...
## Backend full-text search for History — [issue #13](https://github.com/brownfrosamurai/mutter/issues/13)
...
## LLM-based (Option B style) spoken-corrections/spoken-formatting — [issue #14](https://github.com/brownfrosamurai/mutter/issues/14)
...
## Onboarding's mic native-prompt path — root cause found 2026-09-01, still needs a human to verify the fix — [issue #15](https://github.com/brownfrosamurai/mutter/issues/15)
...
## Onboarding Skip may need its own framing now that permissions are automatic — [issue #16](https://github.com/brownfrosamurai/mutter/issues/16)
...
## Possible dialog overlap: Accessibility and Screen Recording requests don't block like Mic's does — [issue #17](https://github.com/brownfrosamurai/mutter/issues/17)
...
**First fix attempt (documentation only) proved insufficient.** `CONTRIBUTING.md`'s merge-strategy note only named `release/`/`hotfix/` PRs for the merge-commit requirement — it never covered the "trivial version bump — merge `develop` → `main` directly" escape hatch in the branching table above it, which is exactly the undocumented path that got squash-merged (PR #24) and caused this. Rewrote that section to explicitly require a real merge commit for *any* PR into `main`. This didn't hold: PR #26 and PR #31 both got squash-merged again despite the documented instruction (once even via `gh pr merge --merge --admin`, an explicit CLI flag forcing merge-commit mode) — GitHub's merge API appears to silently collapse a *zero-content-diff* merge into a single-parent commit regardless of the requested method, a platform behavior no amount of process documentation can route around.

**Real fix, 2026-09-02: turned off the two branch-protection settings actually causing the pain**, rather than continuing to fight GitHub's merge API:
- `main`'s `required_status_checks.strict` → `false` (`gh api -X PATCH .../branches/main/protection/required_status_checks -F strict=false ...`). This was the setting requiring exact ancestry in the first place — with it off, a `develop` → `main` PR only needs passing CI + review, not a literal ancestor relationship, so the single-parent-commit artifacts GitHub keeps producing no longer block anything.
- `develop`'s `required_linear_history` → `false` (`gh api -X DELETE .../branches/develop/protection/required_linear_history`). This was hiding the "Create a merge commit" option in GitHub's PR UI for any PR targeting `develop`, forcing every `main` → `develop` sync into a manual bypass push (`git push origin <local-branch>:develop`, admin-only) instead of a normal reviewable PR merge.
