...
The actual frosted-glass effect requires real macOS `NSVisualEffectView`/`NSGlassEffectView` vibrancy underneath the CSS — `backdrop-filter` alone in a transparent WKWebView window blurs only the page's own render tree, not the real desktop. The two windows now use genuinely different mechanisms, for real reasons, not by accident:

- **Pill:** still the original Objective-C shim (`native/vibrancy_mask_shim.m`), masking `NSVisualEffectView` via the public `-[NSVisualEffectView setMaskImage:]` API, driven by real-time layout geometry the frontend reports on every content-size change (`reportPillVibrancyLayout()` → `set_pill_vibrancy_layout`). This works reliably for the pill specifically because `session::apply_pill_layout` resizes the *window itself* to exactly fit content on every state change, eliminating dead space — see that function's own docs.
- **Dashboard (2026-09-01, replacing an earlier declarative `windowEffects` config):** `vibrancy::apply_dashboard_glass`, called once at startup from `lib.rs`'s `setup()` — not per-layout, no frontend reporting needed. On macOS 26+ (Tahoe), this is `window-vibrancy` 0.8's `apply_liquid_glass`, using Apple's real `NSGlassEffectView` with a genuine public `cornerRadius` and a `content_view` option that reparents the WKWebView *into* the glass view. On older macOS it falls back to plain `apply_vibrancy` (flat corners, matching the window's previous declarative behavior). This replaced a same-day maskImage-masking attempt for the dashboard that was live-verified broken via screenshots and native NSLog diagnostics — real research (not another guess) found the actual root cause: `NSVisualEffectView.maskImage` only reliably masks when the effect view *is* the window's content view, not a subview alongside a WKWebView sibling, which is what every prior dashboard-masking attempt (this session's and the original 2026-08-31 two-shape attempt) was actually fighting. `apply_liquid_glass`'s `content_view` reparenting fixes this at the source.

If the squircle-corner follow-up below is ever built for the pill, its geometry needs to flow through the pill's existing reporting path so the native mask matches the CSS clip-path exactly — the dashboard's `NSGlassEffectView.cornerRadius` already gets true continuous-corner rounding for free from AppKit, no clip-path matching needed there.
