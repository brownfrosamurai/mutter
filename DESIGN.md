...
The actual frosted-glass effect requires real macOS `NSVisualEffectView`/`NSGlassEffectView` vibrancy underneath the CSS — `backdrop-filter` alone in a transparent WKWebView window blurs only the page's own render tree, not the real desktop. Both windows now go through the same mechanism, `vibrancy::apply_glass_shell`, called once each at startup from `lib.rs`'s `setup()` — no per-layout frontend reporting needed for either one:
...
If the squircle-corner follow-up below is ever built, `NSGlassEffectView.cornerRadius` already gets true continuous-corner rounding for free from AppKit on both windows — no CSS clip-path matching needed on the native side.
