...
| `--surface-filled` | `rgba(139, 124, 246, 0.55)` | Toggle's on-track, activity chart's wave stroke + today-dot halo, onboarding's active progress dot, PermissionRow's Grant button and Onboarding/Continue CTA fill |
...
| `--surface-track` | `rgba(255, 255, 255, 0.08)` | Onboarding's inactive progress dots |
...
## Implementation notes (native rendering)

The actual frosted-glass effect requires real macOS `NSVisualEffectView`/`NSGlassEffectView` vibrancy underneath the CSS — `backdrop-filter` alone in a transparent WKWebView window blurs only the page's own render tree, not the real desktop. All four windows go through the same mechanism, `vibrancy::apply_glass_shell`, called once each at startup from `lib.rs`'s `setup()` — no per-layout frontend reporting needed for any of them:

- **Mechanism:** on macOS 26+ (Tahoe), `window-vibrancy` 0.8's `apply_liquid_glass`, using Apple's real `NSGlassEffectView` with a genuine public `cornerRadius` and a `content_view` option that reparents the WKWebView *into* the glass view. On older macOS it falls back to plain `apply_vibrancy` (flat, unrounded vibrancy). This replaced an earlier per-window `NSVisualEffectView.maskImage` approach (a hand-built CoreGraphics mask, re-applied on every layout change) that a same-day investigation (2026-09-01) found was live-verified broken for the dashboard via screenshots and native NSLog diagnostics — real research (not another guess) found the actual root cause: `maskImage` only reliably masks when the effect view *is* the window's content view, not a subview alongside a WKWebView sibling, which every maskImage-based attempt in this app's history was actually fighting. `apply_liquid_glass`'s `content_view` reparenting fixes this at the source.
- **Dashboard:** radius `DASHBOARD_SHELL_RADIUS` (16px, matching `--radius-panel`). Its dead space (the gap around the floating sidebar) is a deliberate layout element it can't shrink away, so that gap shares the same glass shell as the sidebar and card — see App.tsx's module doc for that trade.
- **Pill (migrated 2026-09-01, replacing its own working-but-fragile `maskImage` shim):** radius `PILL_SHELL_RADIUS` (`session::PILL_HEIGHT / 2`, a constant — the pill's height never changes, only its width). Unlike the dashboard, the pill has no dead-space trade-off to make at all: `session::resize_pill_to_content` already resizes the *window itself* to exactly fit `#pill`'s content on every state change (originally added to fix a blur-bleed "wisp" past the old mask's hard alpha edge), so the window's content view is always precisely the capsule shape already. A single constant radius applied once therefore has nothing left to get wrong, no re-masking on every content-size change, and no more wisp — see `vibrancy.rs`'s module doc for the full before/after.
- **Onboarding and Recovery (migrated 2026-09-01, `/design-consultation`):** both previously ran a full generation behind dashboard/pill — declarative `windowEffects: hudWindow` in `tauri.conf.json` (flat vibrancy, no true squircle corners) plus native `decorations: true` (real macOS traffic lights + title bar), while dashboard/pill were already fully chromeless. `GlassPanel.tsx`'s `.glass-panel` CSS material (lensing/tint/morph) was already shared by all four windows — only the *native* layer diverged, which is what actually reads as "this window feels different." Fixed by giving both the same treatment as dashboard: `apply_glass_shell` at radius `16px` (matching `--radius-panel`), `decorations: false`. Both needed a `data-tauri-drag-region` element added (previously relied on the native title bar for window dragging, same requirement `Pill.tsx`/`App.tsx` already satisfy). Recovery already has its own in-app Quit button (`Recovery.tsx`, `commands.quitApp()`) so losing the traffic light's quit affordance costs nothing there; Onboarding never had a quit button and doesn't need one — the tray icon's Quit menu item is already this app's primary quit path given its `ActivationPolicy::Accessory` design, not the traffic light.

If the squircle-corner follow-up below is ever built, `NSGlassEffectView.cornerRadius` already gets true continuous-corner rounding for free from AppKit on all four windows — no CSS clip-path matching needed on the native side.
...
| 2026-09-01 | Dashboard window default size 600×460 → 520×400; `shadow: false` added (was the one window still leaving it unset) | User-directed size reduction; shadow removal matches pill's existing config, real native drop-shadow was following the window's rectangular frame regardless of any CSS/vibrancy masking |
| 2026-09-01 | Dashboard's square "shell" corners fixed via `window-vibrancy` 0.8's `apply_liquid_glass` (macOS 26+ real `NSGlassEffectView`, `content_view` reparenting), flat-vibrancy fallback below macOS 26 | A same-day `maskImage`-based masking attempt was live-verified broken (correct geometry, no visual effect) and reverted; real research (not another guess) found the actual root cause and the actual fix — see "Implementation notes" above for the full mechanism |
| 2026-09-01 | Pill migrated onto the same `apply_liquid_glass`/`apply_glass_shell` mechanism as the dashboard, retiring its own separate `maskImage` shim (`native/vibrancy_mask_shim.m`, deleted) | User-directed ("clean up the shell of the pill the same way we cleaned up the dashboard"); the pill's fixed-height, resize-to-content window means a single constant corner radius has no dead-space trade-off to make at all, unlike the dashboard — see "Implementation notes" above |
| 2026-09-01 | Onboarding and Recovery migrated onto `apply_glass_shell` (radius `16px`) and made chromeless (`decorations: false`), matching dashboard/pill | User-directed, `/design-consultation`: both windows were a full generation behind — CSS material was already shared via `GlassPanel`, but the native vibrancy mechanism and window chrome weren't, which is what actually read as "this window feels different." Requires adding `data-tauri-drag-region` to both (previously relied on the native title bar); Recovery already has an in-app Quit button so no quit-affordance gap, Onboarding relies on the tray's existing Quit item (already the app's primary quit path) |
| 2026-09-02 | Activity chart rebuilt from 7 discrete equal-width bars to one smooth SVG spline (Catmull-Rom → cubic-Bezier) with a violet gradient fill underneath, `ActivityChart.tsx` | User-directed, `/design-consultation` (D1: smooth line/area wave chosen over an audio-waveform-bars alternative, to avoid visually colliding with the pill's own real-time listening waveform, which represents something different — live audio, not historical counts). Reuses `--surface-filled`'s existing violet token for the stroke/dot-halo rather than introducing a new one. Found and fixed a real live-verified WebKit bug along the way: the natural implementation (SVG `pathLength="1"` + `vector-effect="non-scaling-stroke"` for the "draw itself in" entrance) renders as a genuinely dashed line under this chart's non-uniform `preserveAspectRatio="none"` scale — `non-scaling-stroke` reinterprets `stroke-dasharray` in screen pixels instead of the path's user-space units, breaking the normalized-length assumption. Fixed by dropping `pathLength`/`vector-effect` entirely and measuring the real path length via `getTotalLength()` at mount (the traditional, engine-independent version of the same draw-in technique), and moving the today-marker dot out of the SVG into a plain absolutely-positioned HTML div (a `<circle>` inside this same non-uniformly-scaled viewBox renders as a visibly squashed ellipse, not a circle). Verified live via `cargo tauri build --debug` against real history data, not just `npm run build` passing — see `ActivityChart.tsx`'s doc comment for the full mechanism. |
| 2026-09-02 | Dashboard window default width `520px` → `640px` (`tauri.conf.json`) | User-directed size increase; `minWidth: 420` and the resizable/shadow-off config are unchanged |
| 2026-09-02 | Dashboard window default height `400px` → `460px` (`tauri.conf.json`) | User-directed — the Metrics panel's 4-tile grid + Activity wave + 2-row Latency table needed ~403px of natural, unpadded content height at `640px` wide, so `400` clipped the bottom of the Latency table's last row. Determined empirically (live-resized the running window, screenshotted at several heights) rather than guessed; `460` gives ~57px of real breathing room below the table. `minHeight: 320` is unchanged — a manually-shrunk window still scrolls (`overflow-y-auto`) rather than clipping outright |
